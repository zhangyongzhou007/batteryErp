...
public class StringUtils {
    ...
    private static SimpleDateFormat sdf = new SimpleDateFormat(df);
     ...
     * <p>
     * In Jive, every time a batteryErp logs in, we simply
     * take their plain text password, compute the hash, and compare the
     * generated hash to the stored hash. Since it is almost impossible that
     * two passwords will generate the same hash, we know if the batteryErp gave us
     ...
     * </p>
     * <p>In Jive, every time a batteryErp logs in, we simply
     * take their plain text password, compute the hash, and compare the
     * generated hash to the stored hash. Since it is almost impossible that
     * two passwords will generate the same hash, we know if the batteryErp gave us
     * the correct password or not. The only negative to this system is that
...
/*    public static byte[] decodeBase64(String data) {
	 ...
	 * 
	 * @param input The source string
	 * @return  result by string is null 
	 */
	 ...
	 * @throws
	 */
	public static String getString(Long value,int bytes){
		String strvalue = String.valueOf(value);
		if(strvalue.length() > bytes){
			strvalue = strvalue.substring(strvalue.length()-bytes);
		}else {
			...
			strvalue = tmp.substring(strvalue.length()) + strvalue;
		}
		return strvalue;
	}
	/**
	 * 将时间转换为yyyy-MM-dd HH:mm:ss.SSS格式字符串
	 * @param date
	 * @return
	 */
	...
	public static String print(String... strs){
...
//	}
	...
	public static <T1,T2> String print(Map<T1, T2> map){
		StringBuilder sb = new StringBuilder();
	...
	public static <T> String print(List<T> ts){
		StringBuilder sb = new StringBuilder();
		for(int i=0;i<ts.size();i++){
			sb.append("[");
			sb.append(ts.get(i).toString());
			sb.append("]");
		}
		return sb.toString();
	}
	...
	public static String null2String(String str) {
		return str == null || str == "null" || str == ""  ? "" : str.toString();
	}
    ...
    public static String create5BitStr(Long sort) {
        return create5BitStr(String.valueOf(sort));
    }
    public static String create5BitStr(Integer sort) {
	    return create5BitStr(String.valueOf(sort));
    }

    public static String create5BitStr(String sort) {
        StringBuffer sb = new StringBuffer(5);
        int length = sort.length();
        for (int i = 0; i < (5 - length); i ++) {
            sb.append("0");
        }
        return sb.append(sort).toString();
    }
    ...
    public static String listToString(List<String> list) {
    	...
    	return removeHeadAndTailComma(sb.toString());
    }
	 ...
	 * 
	 * @Title: removeComma 
	 * @Description: 去除字符串首尾处的逗号 
	 * @param @param s
	 * @param @return    
	 * @return String  
	 * @throws
	 */
	public static String removeHeadAndTailComma(String reqString)
	{
...
//    	String urlO = url.replaceAll(":", "%3A");
//    	String urlT = urlO.replaceAll("/", "%2F");
    	String eurl = null;
		try {
     ...
     * @return
     */
    public static String formateParamUrl(Map<String,String> paramMap){
    	Set<Entry<String, String>> set = paramMap.entrySet();
    	List<Entry<String, String>> lists = new ArrayList<Entry<String, String>>(set);
    	...
    	StringBuffer buffer = new StringBuffer();
    	for(Entry<String, String> enty : lists){
    ...
    public static String getFormatString(Collection<String> collection){
    ...
    public static String[] getIncludeFields(String includeFields){
    	if(isBlank(includeFields)){
    		return new String[]{};
    	}
	...
	public static String getRegexGroup(String str,String regex){
		
		Pattern varPattern = Pattern.compile(regex);
	...
	public static ArrayList<String> getRegexGroups(String str,String regex){
		ArrayList<String> groups = new ArrayList<String>();
		Pattern varPattern = Pattern.compile(regex);
		Matcher varMatcher = varPattern.matcher(str);
		while (varMatcher.find()) {
			groups.add(varMatcher.group(1));
		}
		return groups;
	}
	
	public static boolean regexMatch(String str,String regex,boolean ignoreCase){
		Pattern varPattern = null;
		if (ignoreCase){
			varPattern = Pattern.compile(regex,Pattern.CASE_INSENSITIVE);
		}
		else{
			varPattern = Pattern.compile(regex);
		} 
		Matcher varMatcher = varPattern.matcher(str);
		if (varMatcher.find()) {
			return true;
     ...
     * @return
     */
    public static String getStringDuration(long second){
